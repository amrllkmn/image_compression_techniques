...
def count_hilbert(array):
    array = array.reshape((-1, 3))
    values = chain.from_iterable(array)
    counts = Counter(values).items()
...
def encode_hilbert(hilbert, codes, bitstream):
    hilbert = hilbert.reshape((-1, 3))
    for pixel in hilbert:
        for value in pixel:
            bitstream.write_bits(codes[value])
...
def decode_array(height, width, tree, bitstream):
    pixels = np.zeros((height * width * 3))
    for i in range(height * width * 3):
        pixels[i] = decode_value(tree, bitstream)
